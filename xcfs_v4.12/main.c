...
static int xcfs_read_super(struct super_block *sb, void *raw_data, int silent)
{
	...
	struct inode *inode;
  printk(KERN_INFO "XCFS_READ_SUPER");
	if (!dev_name) {
		printk(KERN_ERR
	...
	d_set_d_op(sb->s_root, &xcfs_dops);
...
struct dentry *xcfs_mount(struct file_system_type *fs_type, int flags,
...
static struct file_system_type xcfs_fs_type = {
	.owner		= THIS_MODULE,
	.name		= XCFS_NAME,
	.mount		= xcfs_mount,
...
static int __init init_xcfs_fs(void)
{
	int err;

	pr_info("Registering xcfs " XCFS_VERSION "\n");

	err = xcfs_init_inode_cache();
	if (err)
		goto out;
	err = xcfs_init_dentry_cache();
	if (err)
		goto out;
	err = register_filesystem(&xcfs_fs_type);
out:
	if (err) {
...
static void __exit exit_xcfs_fs(void)
{
	xcfs_destroy_inode_cache();
	xcfs_destroy_dentry_cache();
	unregister_filesystem(&xcfs_fs_type);
	pr_info("Completed xcfs module unload\n");
}

MODULE_AUTHOR("Erez Zadok, Filesystems and Storage Lab, Stony Brook University"
	      " (http://www.fsl.cs.sunysb.edu/)");
MODULE_DESCRIPTION("xcfs " XCFS_VERSION
		   " (http://xcfs.filesystems.org/)");
MODULE_LICENSE("GPL");

module_init(init_xcfs_fs);
module_exit(exit_xcfs_fs);
